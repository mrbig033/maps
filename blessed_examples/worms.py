...
def left_of(segment, term):
    ...
    return Location(y=segment.y, x=max(0, segment.x - 1))


def right_of(segment, term):
    """Return Location right-of given segment."""
    return Location(y=segment.y, x=min(term.width - 1, segment.x + 1))
...
def above(segment, term):
    ...
    # pylint: disable=unused-argument
    #         Unused argument 'term'
    return Location(y=max(0, segment.y - 1), x=segment.x)


def below(segment, term):
    """Return Location below given segment."""
    return Location(y=min(term.height - 1, segment.y + 1), x=segment.x)
...
def next_bearing(term, inp_code, bearing):
    ...
    return {
        ...
        {LEFT: left_of, RIGHT: right_of, UP: above, DOWN: below}[
            (bearing.y, bearing.x)
        ],
    )
...
def change_bearing(f_mov, segment, term):
    ...
    return Direction(
        f_mov(segment, term).y - segment.y, f_mov(segment, term).x - segment.x
    )
...
def hit_vany(locations, segments):
    ...
    return any(hit_any(loc, segments) for loc in locations)
...
def next_nibble(term, nibble, head, worm):
    ...
    while hit_vany([head] + worm, nibble_locations(loc, val)):
        loc = Location(
            x=randrange(1, term.width - 1), y=randrange(1, term.height - 1)
        )
        val = nibble.value + 1
